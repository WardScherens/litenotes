            ...
            <div class="flex">
                @if(!$note->trashed())
                    ...
                    <form action="{{ route('notes.destroy', $note) }}" method="post">
                        @method('delete')
                        @csrf
                        <button type="submit" class="btn btn-danger ml-4" onclick="return confirm('Are you sure you want to delete this note?')" >Move to trash Note</button>
                    </form>
                @else
                    <p class="opacity-70">
                        <strong>Deleted at: </strong>{{ $note->updated_at->diffForHumans() }}
                    </p>
